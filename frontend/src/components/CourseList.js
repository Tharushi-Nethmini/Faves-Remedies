...
const CourseList = () => {
    ...
    const fetchCourses = async () => {
        try {
            const response = await axios.get('http://localhost:8070/course/getcourses');
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this course?')) {
            try {
                await axios.delete(`http://localhost:8070/course/deleteCourse/${id}`);
    ...
    const handleUpdate = async (id, updatedData) => {
        try {
            await axios.put(`http://localhost:8070/course/updateCourse/${id}`, updatedData);
            fetchCourses(); // Refresh the list
