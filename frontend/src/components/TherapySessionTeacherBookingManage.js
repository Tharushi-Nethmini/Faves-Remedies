...
import DashboardAdmin from "./DashboardNavbar";
...
export default function TherapySessionTeacherBookingManage() {
  ...
  const getBooking = () => {
    axios
      .get("http://localhost:8070/therapy-session-teacher/view-booking")
      .then((res) => {
  ...
  const saveEdit = (bookingid) => {
    axios
      .put(`http://localhost:8070/therapy-session-teacher/update-booking/${bookingid}`, formData)
      .then(() => {
  ...
  const deleteData = (bookingid) => {
    axios
      .delete(`http://localhost:8070/therapy-session-teacher/delete-booking/${bookingid}`)
      .then(() => {
  ...
  return (
    <><><DashboardNavbar /><div className="main-container">
      <div className="content-container">
        <h1 className="page-title">Speech and Behavior occupational Therapy sessions</h1>
                ...
                <tr key={item._id}>
                  ...
                  <td>
                    {editedItem === item._id ? (
                      <input
                        type="number"
                        name="noOfsessionPerweek"
                        value={formData.noOfsessionPerweek || ""}
                        onChange={handleInputChange} />
                    ) : (
                      item.noOfsessionPerweek
                    )}
                  </td>
                  <td>
                    {editedItem === item._id ? (
