...
const DisplayDetails = () => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        const response = await axios.get("http://localhost:8070/category/alldetails");
  ...
  const handleDelete = async (detailId) => {
    if (window.confirm("Are you sure you want to delete this detail?")) {
      try {
        const response = await axios.delete(`http://localhost:8070/category/deletedetails/${detailId}`);
        ...
        {filteredDetails.length > 0 ? (
          filteredDetails.map((detail) => (
            <div key={detail._id} className="detail-card">
              ...
              <p>Profile:</p>
              <img
                src={`http://localhost:8070/${detail.profilePicture}`} 
