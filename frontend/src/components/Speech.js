...
//   // Define a mapping of counselor names (or IDs) to profile pictures
//   const profilePictures = {
//     "Chamika Gunarathna": "chamika.jpeg",
//   };
...
//           "http://localhost:8070/category/counselingdetails?service=Speech and behavior occupational therapy session"
//         );
...
//     if (profilePictures[name]) {
//       return require(`../Assets/${profilePictures[name]}`);
//     }
...
//               {details.map((detail) => {
//                 const profilePicturePath = getProfilePicture(detail.tName);
...
//                       className="speech-therapy-profile-picture"
//                     />
...
const Speech = () => {
  ...
  const getProfilePicture = (name, profileImage) => {
    if (profileImage) {
      return `http://localhost:8070${profileImage}`;  
    }
    console.log("No profile picture found for:", name);
    return require('../assets/default.png');  
  };
            ...
            <ul className="speech-therapy-list">
              {details.map((detail) => {
                ...
                return (
                  <li className="speech-therapy-item" key={detail._id}>
                  <img
                    src={`http://localhost:8070/${detail.profilePicture}`} // Use the full URL for the image
                    alt={`${detail.tName}'s profile`}
